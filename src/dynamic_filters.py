...
class DynamicFiltersWithList(DynamicFilters):
    """
    A class to create dynamic multi-select filters in Streamlit with a list of options.
    """

    def __init__(self, df, filters, filters_name="filters"):
        """
        Constructs all the necessary attributes for the DynamicFiltersWithList object.
        """
        super().__init__(df, filters, filters_name)
    
    def _is_listy_series(self, s: pd.Series) -> bool:
        """Devuelve True si la serie contiene listas/tuplas/sets o strings con formato de lista."""
        if s.empty:
            return False
        def _is_listy(x):
            ...
            return False
        return s.apply(_is_listy).any()

    def _to_list(self, x):
        """Convierte cualquier valor a lista de strings (maneja '[]' como vacío)."""
    ...
    # --- reemplaza estos métodos en tu clase DynamicFilters ---

    def filter_df(self, except_filter=None):
        """
        Filtra el dataframe basado en el estado, soportando columnas list-like.
        """
        filtered_df = self.df.copy()

        for key, values in st.session_state[self.filters_name].items():
            if key == except_filter or not values:
                continue

            col = filtered_df[key]
            if self._is_listy_series(col):
                # Mantén filas donde hay intersección entre la lista de la fila y los valores seleccionados
                sel_set = set(values)
                mask = col.apply(lambda x: len(set(self._to_list(x)).intersection(sel_set)) > 0)
                filtered_df = filtered_df[mask]
            else:
                filtered_df = filtered_df[col.isin(values)]

        return filtered_df

    def display_filters(self, location=None, num_columns=0, gap="small"):
        # --- (validaciones tal como ya las tienes) ---
        if location not in ["sidebar", "columns", None]:
        ...
        # columnas para layout en columnas (igual que tu código)
        if location == "columns" and num_columns > 0:
            counter = 1
            max_value = num_columns
            col_list = st.columns(num_columns, gap=gap)
        ...
        for filter_name in st.session_state[self.filters_name].keys():
            # DF filtrado por el resto de filtros
            filtered_df = self.filter_df(filter_name)
            col = filtered_df[filter_name]

            # --- opciones: soporta list-like ---
            if self._is_listy_series(col):
                opts_set = set()
                for v in col:
                    for x in self._to_list(v):
                        if x != "":
                            opts_set.add(x)
                options = sorted(opts_set)
            else:
                options = sorted([x for x in col.dropna().unique().tolist()])

            # depura selecciones inválidas
            valid = [v for v in st.session_state[self.filters_name][filter_name] if v in options]
            if valid != st.session_state[self.filters_name][filter_name]:
                st.session_state[self.filters_name][filter_name] = valid
                filters_changed = True

            # dibuja el multiselect
            widget = st.sidebar if location == "sidebar" else None
            if location == "columns" and num_columns > 0:
                widget = col_list[counter - 1]

            with (widget if widget is not None else st):
                selected = st.multiselect(
                    f"Select {filter_name}",
                    options,
                    default=st.session_state[self.filters_name][filter_name],
