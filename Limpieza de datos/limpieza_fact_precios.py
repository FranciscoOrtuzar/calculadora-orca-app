...
MANUAL_RANGE: Optional[Tuple[str,str]] = None  # ("O", "W") o None
...
def find_price_block(raw: pd.DataFrame,
    ...
    # fila de rótulos de secciones (tu "fila 5"): 2 filas arriba de los meses
...
raw = pd.read_excel(INPUT, header=None, dtype=str, engine="openpyxl")
...
price_cols = find_price_block(raw, month_row, PRICE_LABELS, MANUAL_RANGE)

# 3) Leer datos con header en la fila de meses para extraer valores por índice
df = pd.read_excel(INPUT, dtype=str, header=month_row, engine="openpyxl")
