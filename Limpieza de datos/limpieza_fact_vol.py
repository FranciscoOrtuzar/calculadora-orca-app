...
SHEET_NAME = "Ventas (KG)"
OUTPUT = "FACT_VOL.xlsx"
...
def find_sku_col(raw: pd.DataFrame, month_row: int) -> int:
    """
    Intenta encontrar la columna de SKU por:
    1) Buscar 'Codigo Sisfrigo' o 'SKU' en la fila anterior a meses (month_row-1).
    2) Si no, buscar en la fila de meses.
    3) Si no, usa la primera columna.
    """
    label_row = month_row - 1
    candidates = []
    needles = ["codigo sisfrigo", "sku", "código sisfrigo"]
    if label_row >= 0:
        for j in range(raw.shape[1]):
            v = norm_low(raw.iat[label_row, j]) if j < raw.shape[1] else ""
            if any(n in v for n in needles):
                candidates.append(j)
    if not candidates:
        # intenta en la fila de meses por si quedó ahí
        for j in range(raw.shape[1]):
            v = norm_low(raw.iat[month_row, j])
            if any(n in v for n in needles):
                candidates.append(j)
...
sku_col_idx = find_sku_col(raw, month_row)
